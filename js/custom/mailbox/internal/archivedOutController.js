...
(function () {
  ...
  angular
    .module('app.mailbox')
    .controller('archivedOutController', archivedOutController);

  archivedOutController.$inject = [
    '$stateParams',
    '$state',
    '$rootScope',
    'mailboxFactory',
  ...
  function archivedOutController(
    $stateParams,
    $state,
    $rootScope,
    mailboxFactory,
      ...
      Profile.getProfile().then(function (profile) {
        ...
        if (!vm.profile.jobTitleID) {
          ...
          if ($stateParams && $stateParams.folder) {
            vm.paginator.init('archivedInOutFolder');
    ...
    function hasChanged() {
      ...
      $state.go('app.mailbox.internal.archivedOut.folder', {
    ...
    function _getAllFolders() {
      Folder.getAllOutFolders({}, function (err, solve) {
        vm.allFolders = solve;
      });
    }

    function _getFavoriteFolders() {
      Folder.getFavoriteOutFolders({}, function (err, solve) {
    ...
    function goToRoot() {
      ...
      vm.onRoot = true;
      vm.currentFolder = null;
      Folder.getRootFoldersOut({}, function (err, solve) {
        vm.internalFolders = solve;
        for (let i = 0; i < vm.internalFolders.length; i++) {
          Folder.getInfoOfFolder(vm.internalFolders[i], function (errdos, solvedos) {
            vm.internalFolders[i]['internalFolders'] = solvedos;
